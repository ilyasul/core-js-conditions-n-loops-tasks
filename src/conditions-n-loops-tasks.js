...
function convertToRomanNumerals(num) {
  ...
  if (num > 10) {
    let i = 1;
    let temp = objectNumbers[10];
    while (i < Math.floor(num / 10)) {
      i += 1;
      temp += objectNumbers[10];
    }
    temp += objectNumbers[num % 10];
    return temp;
  }
  return objectNumbers[num];
}
...
function isPalindrome(str) {
  let revertStr = str[str.length - 1];
...
function getBalanceIndex(arr) {
  ...
  for (let i = 1; i < arr.length; i += 1) {
    tempSum += arr[i - 1];
    const checkNumb = sum - arr[i] - tempSum;
    if (tempSum === checkNumb) {
      index = i;
      return index;
    }
  }
